...
function _parse(src, filePath, sourceType) {
  const isTS = /\.ts$/.test(filePath);
  const isUsingSyntaxExtension = /\.(jsx|tsx)$/.test(filePath);
  ...
  if (isUsingSyntaxExtension || isTS) {
    plugins.push(isUsingSyntaxExtension ? 'jsx' : 'typescript');
  }
